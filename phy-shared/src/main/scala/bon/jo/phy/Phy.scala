...
object Phy :
  ...
  trait _XYT :
    ...
    def to[Other <: XYT[_]](using fact: Fact[Other]): Other = fact(x, y)


  abstract class XYT[Self <: XYT[_]](using fact : Fact[Self]) extends _XYT :
  ...
  extension( d: Double)
    def *[XY <: XYT[_]](xy: XY)(using f: Fact[XY]): XY =
      f(d * xy.x, d * xy.y)
