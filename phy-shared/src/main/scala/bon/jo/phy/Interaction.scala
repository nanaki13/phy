...
abstract sealed  class Interaction(val name: String):
  ...
  def correction(aSans: A)(implicit ctx: CaculContext, e: CalculParam): A =
    if e.correction then
      if(ctx.dist > ctx.rSup) then
        val PS:A = ctx.dist * ctx.pointToSource
        val rr :A = ctx.rSup/20 * ctx.pointToSource.rotate90
        val cheminV :A =  PS + rr
        (ctx.rSup/20  * cheminV.unitary) - 0.1 * ctx.point.v.to[A]
      else if(ctx.dist < ctx.rInf) then
          A() - 0.5 * ctx.point.v
      else
        aSans
    else
